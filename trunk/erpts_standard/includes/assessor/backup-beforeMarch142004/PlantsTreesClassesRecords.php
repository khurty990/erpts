<?php
class PlantsTreesClassesRecords
{
	var $arrayList;
	var $domDocument;
	var $db;
	
	function PlantsTreesClassesRecords(){
	...
	function setDomDocument(){
		...
		$domList = $this->domDocument->create_element("PlantsTreesClassesList");
	...
	function parseDomDocument($domDoc){
		...
		if ($baseNode->has_child_nodes()){
			...
			while ($child){
				//if ($child->tagname=="PlantsTreesClasses") {
				if ($child->tagname) {
					$tempXmlStr = $domDoc->dump_node($child);
					$tempDomDoc = domxml_open_mem($tempXmlStr);
					$plantsTreesClasses = new PlantsTreesClasses;
					$plantsTreesClasses->parseDomDocument($tempDomDoc);
					$this->arrayList[] = $plantsTreesClasses;
				}
	...
	function selectRecords($condition=""){
		...
		//		PLANTSTREESCLASSES_TABLE, $condition);

		$sql = sprintf("select 
			plantsTreesClassesID,
			code,
			description,
			value as unitValue,
			status			 
		...
		while ($this->db->next_record()) {
			$plantsTreesClasses = new PlantsTreesClasses;
			$plantsTreesClasses->selectRecord($this->db->f("plantsTreesClassesID"));
			$this->arrayList[] = $plantsTreesClasses;
		}
	...
	function searchRecords($searchKey,$fields,$limit){
		...
		$sql = sprintf("select * from %s %s;",
				PLANTSTREESCLASSES_TABLE, $condition.") ".$limit);
		//echo $sql;
		$this->setDB();
		$this->db->query($sql);
		while ($this->db->next_record()) {
	...
	function deleteRecords($plantsTreesClassesIDArray=""){
		$plantsTreesClasses = new PlantsTreesClasses;
		$rows = 0;
		foreach ($plantsTreesClassesIDArray as $key => $value){
			if ($plantsTreesClasses->deleteRecord($value)) $rows++;
		}
		return $rows;
	}
	...
	function updateStatus($statusIDArray=""){
	    ...
	    foreach($statusIDArray as $key => $value){
	        $plantsTreesClasses = new PlantsTreesClasses;
	        $plantsTreesClasses->selectRecord($value);
	        $plantsTreesClasses->status = "active";
	        $plantsTreesClasses->updateRecord();
	        $rows++;
	    }
	    return $rows;
	}
