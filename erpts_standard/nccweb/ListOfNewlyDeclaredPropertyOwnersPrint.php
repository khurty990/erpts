...
#####################################
class ListOfNewlyDeclaredPropertyOwnersPrint{
	
	var $tpl;
	var $formArray;
	var $sess;

	function ListOfNewlyDeclaredPropertyOwnersPrint($http_post_vars,$sess,$formAction,$formArray,$page=1){
		global $auth;
		...
		$this->tpl->set_file("rptsTemplate", "ListOfNewlyDeclaredPropertyOwnersPrint.htm") ;
		$this->tpl->set_var("TITLE", "List of Newly Declared Property Owners");
	...
	function Main(){
		switch ($this->formArray["formAction"]){
			case "view":
				...
				$condition .= " AND (".OD_TABLE.".transactionCode = 'DC' OR ".OD_TABLE.".transactionCode = '')";
...
$obj = new ListOfNewlyDeclaredPropertyOwnersPrint($HTTP_POST_VARS,$sess,$formAction,$formArray,$page);
$obj->main();
?>
<?php page_close(); ?>
