...
class TDDetails{
	...
	function setForm(){
		...
		$this->formatCurrency1("bfindingsFloorArea");
		$this->formatCurrency1("grandTotalArea");
		//end
        ...
        if (count($improvementsBuildingsList)){
			...
			foreach ($improvementsBuildingsList as $key => $improvementsBuildings){
				...
				if($i<3){
				 ...
				 //$bFindingsFloorArea +=toFloat($land->getAreaOfGroundFloor());
				 //$this->formArray["unit"] = $land->getUnit();
				}
			...
			$this->formArray["bfindingsFloorArea"] = $bFindingsFloorArea;
		}
	}
