...
class ODDetails{
	...
	function ODDetails($http_post_vars,$sess,$odID,$transactionCode=""){
		...
		$this->tpl->set_var("TITLE", "Subdivision > RPU Details");
	...
	function splitOD(){
		...
		for($i=0 ; $i<$numberToSplit ; $i++){
			$newOdIDArray[] = $RPUEncode->CreateNewRPU_AFS_TD($this->formArray["odID"], $this->formArray["uid"], $this->formArray["transactionCode"]);
		}
	}
	...
	function Main(){
		...
		if (!$xmlStr = $ODDetails->getOD($this->formArray["odID"])){
			...
			if(!$domDoc = domxml_open_mem($xmlStr)) {
				...
				$this->formArray["propertyType"] = $this->getPropertyTypeFromOD($od);

				if($this->formArray["propertyType"]=="") $this->formArray["propertyType"] = "none";
