...
class ODDetails{
	...
	var $tpl;
	var $formArray;
	function ODDetails($http_post_vars,$sess,$odID,$transactionCode=""){
		global $auth;
		...
		$this->tpl->set_var("TITLE", "Auction out Property > RPU Details");
		
		$this->formArray["odID"] = $odID;

		$this->formArray["transactionCodeDone"] = $transactionCode;
		if($this->formArray["transactionCodeDone"]!=""){
			$this->formArray["transactionCodeDoneMsg"] = "This property has been marked with transaction code: <b>" . $this->formArray["transactionCodeDone"] . "</b>.<br>You may update the names of the owner as well as other property information now or upon actual Auction.";
		}
		else{
			$this->formArray["transactionCodeDoneMsg"] = "";
		}
...
//*/
$ownerList = new ODDetails($HTTP_POST_VARS,$sess,$odID,$transactionCode);
$ownerList->main();
?>
<?php page_close(); ?>
