<?php
class ImprovementsBuildingsActualUsesRecords
{
	var $arrayList;
	var $domDocument;
	var $db;
	
	function ImprovementsBuildingsActualUsesRecords(){
	...
	function parseDomDocument($domDoc){
		...
		if ($baseNode->has_child_nodes()){
			...
			while ($child){
				//if ($child->tagname=="ImprovementsBuildingsActualUses") {
				if ($child->tagname) {
					$tempXmlStr = $domDoc->dump_node($child);
					$tempDomDoc = domxml_open_mem($tempXmlStr);
					$improvementsBuildingsActualUses = new ImprovementsBuildingsActualUses;
					$improvementsBuildingsActualUses->parseDomDocument($tempDomDoc);
					$this->arrayList[] = $improvementsBuildingsActualUses;
				}
	...
	function selectRecords($condition=""){
		...
		//		IMPROVEMENTSBUILDINGS_ACTUALUSES_TABLE, $condition);
		...
		while ($this->db->next_record()) {
			$improvementsBuildingsActualUses = new ImprovementsBuildingsActualUses;
			$improvementsBuildingsActualUses->selectRecord($this->db->f("improvementsBuildingsActualUsesID"));
			$this->arrayList[] = $improvementsBuildingsActualUses;
		}
	...
	function searchRecords($searchKey,$fields,$limit){
		...
		$sql = sprintf("select * from %s %s;",
				IMPROVEMENTSBUILDINGS_ACTUALUSES_TABLE, $condition.") ".$limit);
		//echo $sql;
		$this->setDB();
		$this->db->query($sql);
		while ($this->db->next_record()) {
	...
	function deleteRecords($improvementsBuildingsActualUsesIDArray=""){
		$improvementsBuildingsActualUses = new ImprovementsBuildingsActualUses;
		$rows = 0;
		foreach ($improvementsBuildingsActualUsesIDArray as $key => $value){
			if ($improvementsBuildingsActualUses->deleteRecord($value)) $rows++;
		}
		return $rows;
	}
	...
	function updateStatus($statusIDArray=""){
	    ...
	    foreach($statusIDArray as $key => $value){
	        $improvementsBuildingsActualUses = new ImprovementsBuildingsActualUses;
	        $improvementsBuildingsActualUses->selectRecord($value);
	        $improvementsBuildingsActualUses->status = "active";
	        $improvementsBuildingsActualUses->updateRecord();
	        $rows++;
	    }
	    return $rows;
	}
