			...
			if(!$domDoc = domxml_open_mem($xmlStr)) {
				...
				if (count($list)){
					...
					foreach ($list as $key => $value){
						...
						# $buffer[$i]["taxableYear"]=$forDate['year'];
						...
						*/
						$oValue = $value->owner;
						$buffer[$i]["rptopOwnerID"]=$oValue->getOwnerID();
						$pOwnerStr = "";
					...
					for ($i=0; $i<count($buffer);$i++) {
						...
						# get earliest taxable year for each RPTOP
						$currentYear = date('Y');
						echo("current year ".$currentYear."<br>");
						$earliestTaxableYear = $rptop->getEarliestTaxableYear($buffer[$i]["rptopOwnerID"]);
						echo($earliestTaxableYear."<br>");

						for($j = $earliestTaxableYear; $j <= $currentYear; $j++){
                            $this->tpl->set_var("taxableYear",$j);
							
							if($j == $currentYear)
								$this->tpl->set_var("taxableYearSelect","selected");
							else
								$this->tpl->set_var("taxableYearSelect","");
							$this->tpl->parse("TaxableYearBlock","TaxableYear",true);	
						}
						
						$this->tpl->parse("RPTOPListBlock", "RPTOPList", true);
						$this->tpl->set_var("TaxableYearBlock", "");
