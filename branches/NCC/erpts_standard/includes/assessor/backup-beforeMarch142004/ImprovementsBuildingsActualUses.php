...
class ImprovementsBuildingsActualUses
{
	//attributes
	var $improvementsBuildingsActualUsesID;
	var $code;
	var $reportCode;
	var $description;
	var $rangeLowerBound;
	...
	//constructor
	function ImprovementsBuildingsActualUses() {
	...
	function setImprovementsBuildingsActualUsesID($tempVar) {
		$this->improvementsBuildingsActualUsesID = $tempVar;
	}
	...
	function setDomDocument() {
		...
		$rec = $this->domDocument->create_element("ImprovementsBuildingsActualUses");
		$rec = $this->domDocument->append_child($rec);
		//$rec->set_attribute("improvementsBuildingsActualUsesID",$this->improvementsBuildingsActualUsesID);
		$this->setDocNode("improvementsBuildingsActualUsesID",$this->improvementsBuildingsActualUsesID,$this->domDocument,$rec);
	...
	//get
	function getImprovementsBuildingsActualUsesID() {
		return $this->improvementsBuildingsActualUsesID;
	}
	function getCode() {
		return $this->code;
	}
	...
	//DB
	function selectRecord($improvementsBuildingsActualUsesID){
		if ($improvementsBuildingsActualUsesID=="") return;

		$this->setDB();
		$sql = sprintf("SELECT * FROM %s WHERE improvementsBuildingsActualUsesID=%s;",
			IMPROVEMENTSBUILDINGS_ACTUALUSES_TABLE, $improvementsBuildingsActualUsesID);
			$this->db->query($sql);
		$improvementsBuildingsActualUses = new ImprovementsBuildingsActualUses;
		if ($this->db->next_record()) {
			//*
			$this->improvementsBuildingsActualUsesID = $this->db->f("improvementsBuildingsActualUsesID");
	...
	function insertRecord(){
		$sql = sprintf("insert into %s (".
			...
			, IMPROVEMENTSBUILDINGS_ACTUALUSES_TABLE
			, fixQuotes($this->code)
		...
		$this->db->query($sql);
		$improvementsBuildingsActualUsesID = $this->db->insert_id();
		if ($this->db->Errno!=0) {
			...
			$ret = $improvementsBuildingsActualUsesID;
		}
	...
	function deleteRecord($improvementsBuildingsActualUsesID){
		$this->setDB();
		$this->db->beginTransaction();
		$this->selectRecord($improvementsBuildingsActualUsesID);
		$sql = sprintf("delete from %s where improvementsBuildingsActualUsesID=%s;",
			IMPROVEMENTSBUILDINGS_ACTUALUSES_TABLE, $improvementsBuildingsActualUsesID);
		$this->db->query($sql);
		$improvementsBuildingsActualUsesRows = $this->db->affected_rows();
		...
		if ($this->db->Errno != 0) {
			...
			$this->db->endTransaction();
			$ret = $improvementsBuildingsActualUsesRows;
		}
		return $ret;
	}
	...
	function updateRecord(){
		...
		$sql = sprintf("update %s set".
			...
			", value = '%s'".
			", status = '%s'".
			" where improvementsBuildingsActualUsesID = '%s';",
			IMPROVEMENTSBUILDINGS_ACTUALUSES_TABLE
			, fixQuotes($this->code)
			, fixQuotes($this->reportCode)
			, fixQuotes($this->description)
			...
			, fixQuotes($this->value)
			, fixQuotes($this->status)
			, $this->improvementsBuildingsActualUsesID
		);
		//echo $sql;
		$this->setDB();
		$this->db->beginTransaction();
