...
class RPTOPDetails{
	...
	function RPTOPDetails($http_post_vars,$sess,$rptopID){
		...
		$this->tpl->set_file("rptsTemplate", "SOADetails_.htm") ;
	...
	function displayBacktaxTD($tdID){
		...
		if (!$xmlStr = $BacktaxTDDetails->getBacktaxTDList($tdID)){
			//echo $tdID."<br>";
			//$this->tpl->set_block("rptsTemplate", "Backtax", "BacktaxBlock");
			...
			if(!$domDoc = domxml_open_mem($xmlStr)) {
				...
				if(count($backTaxTDList)){
					...
					foreach ($backTaxTDList as $key => $val){
						$val->calculatePenalty("2004-03-25");
								...
								foreach($value as $tkey => $tvalue){
									...
									if (!$xmlStr = $DueList->getDueList($tvalue->getTdID())){
										...
										if(!$domDoc = domxml_open_mem($xmlStr)) {
											...
											// compute DISCOUNTS as of currentDate
											...
											// check if today is within the discountPeriod

											if(strtotime($this->now) >= $this->formArray["discountPeriod_Start"] && strtotime($this->now) <= $this->formArray["discountPeriod_End"]){
												$earlyPaymentDiscount["Annual"] = $dueArrayList["Annual"]->getTaxDue() * ($this->formArray["discountPercentage"]/100);
												$earlyPaymentDiscount["Q1"] = $dueArrayList["Q1"]->getTaxDue() * ($this->formArray["discountPercentage"]/100);
												$earlyPaymentDiscount["Q2"] = $dueArrayList["Q2"]->getTaxDue() * ($this->formArray["discountPercentage"]/100);
												$earlyPaymentDiscount["Q3"] = $dueArrayList["Q3"]->getTaxDue() * ($this->formArray["discountPercentage"]/100);
												$earlyPaymentDiscount["Q4"] = $dueArrayList["Q4"]->getTaxDue() * ($this->formArray["discountPercentage"]/100);
											}

											foreach($earlyPaymentDiscount as $dKey => $dValue){
												$this->tpl->set_var("earlyPaymentDiscount[".$dKey."]",formatCurrency($dValue));
											}

											// compute PENALTIES as of currentDate
