...
#####################################
class RPTOPPersonSearch{
	
	var $tpl;
	var $formArray;
	var $sess;
	
	function RPTOPPersonSearch($http_post_vars,$searchKey,$formAction="",$sess,$ownerID="",$rptopID="",$page){
		$this->sess = $sess;
		$this->tpl = new rpts_Template(getcwd(),"keep");

		$this->tpl->set_file("rptsTemplate", "RPTOPPersonSearch.htm") ;
	...
	function Main(){
		switch ($this->formArray["formAction"]){
			...
			case "add":
					...
					//echo $this->formArray["rptopID"].",".$this->formArray["ownerID"].",".$this->formArray["personID"];
					//exit;
					$OwnerList = new SoapObject(NCCBIZ."OwnerList.php", "urn:Object");
					$OwnerList->addOwnerPersonRPTOP($this->formArray["rptopID"],$this->formArray["ownerID"],$this->formArray["personID"]);
					header("location: RPTOPPersonSearchClose.php".$this->sess->url("").$this->sess->add_query(array("rptopID"=>$this->formArray["rptopID"])));
				break;
			default:
				$this->tpl->set_block("rptsTemplate", "Table", "TableBlock");
...
//echo $ownerID;
if(!$page) $page = 1;
$personEncode = new RPTOPPersonSearch($HTTP_POST_VARS,$searchKey,$formAction,$sess,$ownerID,$rptopID,$page);
$personEncode->main();
?>
