...
class NoticeOfAssessmentList{
	...
	var $tpl;
	function NoticeOfAssessmentList($sess,$sortKey,$sortOrder,$page){
		global $auth;
		...
		$this->tpl->set_file("rptsTemplate", "NoticeOfAssessmentList.htm") ;
	...
	function selectRecords($condition=" ORDER BY personName ASC LIMIT 0,10"){
		$this->setDB();
		$sql = sprintf(
				"SELECT DISTINCT(OwnerPerson.personID) as personID, TRIM(CONCAT(Person.lastName, Person.firstName, Person.middleName)) as personName ".
				" FROM OwnerPerson,Person ".
				" WHERE OwnerPerson.personID = Person.personID ".
				" %s;",
				$condition);
...
//*/
$obj = new NoticeOfAssessmentList($sess,$sortKey,$sortOrder,$page);
$obj->Main();
?>
<?php page_close(); ?>
