<?php
class LandClassesRecords
{
	var $arrayList;
	var $domDocument;
	var $db;
	
	function LandClassesRecords(){
	...
	function setDomDocument(){
		...
		$domList = $this->domDocument->create_element("LandClassesList");
	...
	function parseDomDocument($domDoc){
		...
		if ($baseNode->has_child_nodes()){
			...
			while ($child){
				//if ($child->tagname=="LandClasses") {
				if ($child->tagname) {
					$tempXmlStr = $domDoc->dump_node($child);
					$tempDomDoc = domxml_open_mem($tempXmlStr);
					$landClasses = new LandClasses;
					$landClasses->parseDomDocument($tempDomDoc);
					$this->arrayList[] = $landClasses;
				}
	...
	function selectRecords($condition=""){
		...
		while ($this->db->next_record()) {
			$landClasses = new LandClasses;
			$landClasses->selectRecord($this->db->f("landClassesID"));
			$this->arrayList[] = $landClasses;
		}
	...
	function searchRecords($searchKey,$fields,$limit){
		...
		$sql = sprintf("select * from %s %s;",
				LANDCLASSES_TABLE, $condition.") ".$limit);
		//echo $sql;
		$this->setDB();
		$this->db->query($sql);
		while ($this->db->next_record()) {
	...
	function deleteRecords($landClassesIDArray=""){
		$landClasses = new LandClasses;
		$rows = 0;
		foreach ($landClassesIDArray as $key => $value){
			if ($landClasses->deleteRecord($value)) $rows++;
		}
		return $rows;
	}
	...
	function updateStatus($statusIDArray=""){
	    ...
	    foreach($statusIDArray as $key => $value){
	        $landClasses = new LandClasses;
	        $landClasses->selectRecord($value);
	        $landClasses->status = "active";
	        $landClasses->updateRecord();
	        $rows++;
	    }
	    return $rows;
	}
