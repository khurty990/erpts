<?php
class PropAssessUsesRecords
{
	var $arrayList;
	var $domDocument;
	var $db;
	
	function PropAssessUsesRecords(){
	...
	function setDomDocument(){
		...
		$domList = $this->domDocument->create_element("PropAssessUsesList");
	...
	function selectRecords($condition=""){
		$sql = sprintf("select * from %s %s;",
				PROPASSESSUSES_TABLE, $condition);
		$this->setDB();
		$this->db->query($sql);
		while ($this->db->next_record()) {
			$propAssessUses = new PropAssessUses;
			$propAssessUses->selectRecord($this->db->f("propAssessUsesID"));
			$this->arrayList[] = $propAssessUses;
		}
	...
	function searchRecords($searchKey,$fields,$limit){
		...
		$sql = sprintf("select * from %s %s;",
				PROPASSESSUSES_TABLE, $condition.") ".$limit);
		//echo $sql;
		$this->setDB();
		$this->db->query($sql);
		while ($this->db->next_record()) {
	...
	function deleteRecords($propAssessUsesIDArray=""){
		$propAssessUses = new PropAssessUses;
		$rows = 0;
		foreach ($propAssessUsesIDArray as $key => $value){
			if ($propAssessUses->deleteRecord($value)) $rows++;
		}
		return $rows;
	}
