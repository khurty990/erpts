...
class ODDetails{
	...
	var $tpl;
	var $formArray;
	function ODDetails($http_post_vars,$sess,$odID,$transactionCode="",$formAction){
		global $auth;
		...
		$this->formArray["blockDoubleTransaction"] = $transactionCode;
		$this->formArray["formAction"] = $formAction;
		$this->formArray["cancelError"] = "";

		$this->set_file($this->formArray["transactionCode"],$odID);

		$this->tpl->set_var("TITLE", "Change of Owner > RPU Details");
	...
	function set_file($transactionCode,$odID){
		if($this->formArray["transactionCode"]=="CA"){
			$this->tpl->set_file("rptsTemplate", "ODDetailsCancelled.htm");
			return true;
		}
		else{
			$od = new OD;
			if($od->selectRecord($odID)){
				if($od->getTransactionCode()=="CA"){
					$this->tpl->set_file("rptsTemplate", "ODDetailsCancelled.htm");
					return true;
				}
			}
	...
	function enableDisableCancelButton($transactionCode){
		if($transactionCode=="CA"){
			$this->formArray["blockDoubleTransaction"] = "true";
		}
		else{
			if($this->formArray["cancelError"]!=""){
				$this->formArray["blockDoubleTransaction"] = "true";
			}
			else{
				$this->tpl->set_var("", "true");
			}
		}
	}
	...
	function Main(){
		...
		switch ($this->formArray["formAction"]){
			...
			case "remove":
				...
				break;
			case "cancelError":
				$this->formArray["cancelError"] = "Cannot cancel RPU that already has TD encoded.";
			default:
		...
		if (!$xmlStr = $ODDetails->getOD($this->formArray["odID"])){
			...
			if(!$domDoc = domxml_open_mem($xmlStr)) {
				...
				$this->enableDisableCancelButton($od->getTransactionCode());
			}	
		}
...
//*/
$ownerList = new ODDetails($HTTP_POST_VARS,$sess,$odID,$transactionCode,$formAction);
$ownerList->main();
?>
<?php page_close(); ?>
