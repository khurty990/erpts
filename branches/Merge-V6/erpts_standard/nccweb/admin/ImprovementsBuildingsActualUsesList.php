...
class ImprovementsBuildingsActualUsesList{
	...
	var $tpl;
	var $formArray;
	function ImprovementsBuildingsActualUsesList($http_post_vars,$sess,$sortBy,$sortOrder,$hideInactive){
		global $auth;
		...
		$this->tpl->set_file("rptsTemplate", "ImprovementsBuildingsActualUsesList.htm") ;
	...
	function showHideInactive(){
		...
		switch($this->formArray["hideInactive"]){
			...
			default:
				...
				$condition = " WHERE ".IMPROVEMENTSBUILDINGS_ACTUALUSES_TABLE.".status='active' ";
				break;
		}
		return $condition;
	}
	...
	function Main(){
		switch ($this->formArray["formAction"]){
			case "delete":
				if (count($this->formArray["improvementsBuildingsActualUsesID"]) > 0) {
					$ImprovementsBuildingsActualUsesList = new SoapObject(NCCBIZ."ImprovementsBuildingsActualUsesList.php", "urn:Object");
					if (!$deletedRows = $ImprovementsBuildingsActualUsesList->deleteImprovementsBuildingsActualUses($this->formArray["improvementsBuildingsActualUsesID"])){
			...
			case "activate":
			        $ImprovementsBuildingsActualUsesList = new SoapObject(NCCBIZ."ImprovementsBuildingsActualUsesList.php", "urn:Object");
			        if(!$activeRows = $ImprovementsBuildingsActualUsesList->updateStatus($this->formArray["status"])){
...
//*/
$improvementsBuildingsActualUsesList = new ImprovementsBuildingsActualUsesList($HTTP_POST_VARS,$sess,$sortBy,$sortOrder,$hideInactive);
