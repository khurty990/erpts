...
#####################################
class RetainOwnerPerson{
	
	var $tpl;
	function RetainOwnerPerson($sess,$http_post_vars){
		global $auth;
		...
		$this->tpl->set_file("rptsTemplate", "RetainOwnerPerson.htm") ;
		$this->tpl->set_var("TITLE", "Retain Owner Person");
	...
	function addStatusField(){
		$this->setDB();
		$sqlAddStatusField = "ALTER TABLE `Person` ADD `status` VARCHAR( 32 ) AFTER `email`";
	...
	function generateUpdateSQL(){
		if(is_array($this->formArray["personID"])){
			// add status field ONLY if status field does not exist, run only once in a lifetime :P
			$this->addStatusField();

			foreach($this->formArray["personID"] as $disablePersonID){
				if($this->formArray["retain"]!=$disablePersonID){
					$this->setDB();
					$sql = sprintf("SELECT ownerPersonID FROM OwnerPerson WHERE personID='%s'",$disablePersonID);

					if($this->db->query($sql)){
						while ($this->db->next_record()) {
							$ownerPersonIDArray[] = $this->db->f("ownerPersonID");
						}
					}

					foreach($ownerPersonIDArray as $ownerPersonID){
						$updateSQL[] = sprintf("UPDATE OwnerPerson SET personID='%s' WHERE ownerPersonID='%s'",$this->formArray["retain"],$ownerPersonID);

						$rollbackSQL[] = sprintf("UPDATE OwnerPerson SET personID='%s' WHERE ownerPersonID='%s'",$disablePersonID,$ownerPersonID);
					}

					$updateSQL[] = sprintf("UPDATE Person SET status='inactive' WHERE personID='%s'",fixQuotes($disablePersonID));

					$rollbackSQL[] = sprintf("UPDATE Person SET status='' WHERE personID='%s'",fixQuotes($disablePersonID));

					unset($this->db);
					unset($ownerPersonIDArray);
					$disablePersonIDArray[] = $disablePersonID;
			...
			$this->clearPropertyElements();
			unset($this->formArray["personID"]);

			$this->formArray["personID"] = array($this->formArray["retain"]);
			$this->displayRecords();

			unset($this->formArray["personID"]);
			unset($this->arrayList);

			$this->formArray["personID"] = $disablePersonIDArray;
			$this->selectRecords();

			$this->tpl->set_block("rptsTemplate", "DisabledPersonList", "DisabledPersonListBlock");
			...
			foreach($this->arrayList as $person) {
				...
				$this->tpl->parse("DisabledPersonListBlock","DisabledPersonList",true);
			}
...
//*/
$obj = new RetainOwnerPerson($sess,$HTTP_POST_VARS);
$obj->Main();
?>
<?php page_close(); ?>
