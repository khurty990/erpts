...
#####################################
class ListOfRPUsCancelledPrint{
	
	var $tpl;
	var $formArray;
	var $sess;

	function ListOfRPUsCancelledPrint($http_post_vars,$sess,$formAction,$formArray,$page=1){
		global $auth;
		...
		$this->tpl->set_file("rptsTemplate", "ListOfRPUsCancelledPrint.htm") ;
		$this->tpl->set_var("TITLE", "List of RPUs Permanently Cancelled");
	...
	function filterArchives(){
		// commented out because 'CA' is always archived as 'true'
		//$condition = " AND ".OD_TABLE.".archive <> 'true'";

		// filter cancelled RPUS with transactionCode as "CA"
...
//									if($afs->getTaxability()=="Taxable"){
										...
										$this->tpl->set_var("dateModified", date('F j, Y, g:i a',$value->getDateModified()));
...
$listOfRPUsCancelledPrint = new ListOfRPUsCancelledPrint($HTTP_POST_VARS,$sess,$formAction,$formArray,$page);
$listOfRPUsCancelledPrint->Main();
?>
<?php page_close(); ?>
