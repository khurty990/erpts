...
class TDList{
	...
	function TDList($http_post_vars,$sess,$tdID,$page,$searchKey,$formAction,$sortBy,$sortOrder){
		global $auth;
		...
		$this->tpl->set_file("rptsTemplate", "BacktaxesTDList.htm") ;
		$this->tpl->set_var("TITLE", "TM : Manage Backtaxes");
	...
	function displayBacktaxTDBalance(){
		...
		if (!$xmlStr = $BacktaxTDDetails->getBacktaxTDList($this->formArray["tdID"])){
			$this->tpl->set_var("backtaxBalance", "-");
		}
		else{
			//echo $xmlStr;
			if(!$domDoc = domxml_open_mem($xmlStr)) {
				...
				if(count($backTaxTDList)){
					...
					foreach ($backTaxTDList as $key => $val){
						$backtaxBalance += $val->getBalance();
					}
					$this->tpl->set_var("backtaxBalance",number_format($backtaxBalance,2));
				}
				else{
					$this->tpl->set_var("backtaxBalance", "-");
				}
					...
					if(!$domDoc = domxml_open_mem($xmlStr)) {
						...
						if (count($list)){
							...
							foreach ($list as $key => $value){
								...
								$this->formArray["tdID"] = $value->getTdID();
								$this->displayBacktaxTDBalance();
...
if(!$page) $page = 1;
$tdList = new TDList($HTTP_POST_VARS,$sess,$tdID,$page,$searchKey,$formAction,$sortBy,$sortOrder);
$tdList->main();
?>
<?php page_close(); ?>
