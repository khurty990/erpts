...
class AFSList{
	...
	function AFSList($http_post_vars,$sess,$odID,$page,$searchKey,$formAction,$sortBy,$sortOrder,$viewArchives){
		global $auth;
		...
		$this->tpl->set_var("TITLE", "AFS List");
	...
	function Main(){
		switch ($this->formArray["formAction"]){
			case "archive":
				...
				$afsIDArray = $this->formArray["afsIDArray"];
				$archiveValue = $this->formArray["archiveValue"];
				$userID = $this->formArray["uid"];

				if(!$archiveRows = $AFSList->archiveAFS($afsIDArray,$archiveValue,$userID)){
						$this->tpl->set_var("msg", "SOAP failed");
				}
				else{
					$sess = $this->sess->name."=".$this->sess->id;
					header("Location: AFSList.php?".$sess."&viewArchives=".$this->formArray["viewArchives"]);
					exit;
				}
				break;
            case "search":
...
$afsList = new AFSList($HTTP_POST_VARS,$sess,$odID,$page,$searchKey,$formAction,$sortBy,$sortOrder,$viewArchives);
$afsList->main();
?>
<?php page_close(); ?>
