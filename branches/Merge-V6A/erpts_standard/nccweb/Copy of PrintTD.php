	...
	function TDDetails($http_post_vars,$sess,$odID,$ownerID,$afsID,$print){
		...
		$this->tpl->set_var("TITLE", "TD Details");
       	...
       	$this->formArray = array(
			...
			,"memoranda" => "" // TD
			,"propertyType" => "" // TD
			,"provincialAssessor" => "" // TD
			,"municipalAssessor" => "" // TD
			,"totalArea" => "" // TD
		);
...
//			$offset = 470;
			...
			foreach($landList as $lkey => $land){
				...
				$actualUses = new LandActualUses;
				if(is_numeric($land->getActualUse())){
					$actualUses->selectRecord($land->getActualUse());
					$actualUsesDescription = $actualUses->getDescription();
					$actualUsesCode = $actualUses->getCode();
				}
				else{
					$actualUsesDescription = $land->getActualUse();
					$actualUsesCode = $land->getActualUse();
				}
...
//						if (strlen($au) > 0) {
						if ($av > 0) {
							...
							$items .= "<textitem xpos=\"115\" ypos=\"".$offset."\" font=\"Helvetica\" size=\"9\" align=\"left\">".$au."</textitem>";
							$items .= "<textitem xpos=\"198\" ypos=\"".$offset."\" font=\"Helvetica\" size=\"8\" align=\"left\">".number_format($thisArea,($thisUnit==" sqm.")?2:4).$thisUnit."</textitem>";
							...
							$items .= "<lineitem x1=\"115\" y1=\"".$offsetx."\" x2=\"188\" y2=\"".$offsetx."\">blurb</lineitem>";
							$items .= "<lineitem x1=\"198\" y1=\"".$offsetx."\" x2=\"254\" y2=\"".$offsetx."\">blurb</lineitem>";
							$items .= "<lineitem x1=\"264\" y1=\"".$offsetx."\" x2=\"347\" y2=\"".$offsetx."\">blurb</lineitem>";
					...
					if ($land->getUnit() != "square meters") {
						$runArea += $land->getArea() * 10000;
						$thisArea += $land->getArea();
						$thisUnit = " has.";
					}
					else {
						$runArea += $land->getArea();
						$thisArea += $land->getArea();
						$thisUnit = " sqm.";
					}
...
//				}
			...
			if ($finalUnit == 'has.') {
				$this->formArray["totalArea"] = number_format($runArea/10000,4).' has.';
			}
			else {
				$this->formArray["totalArea"] = number_format($runArea,2).' sqm.';
			}
...
//				if($this->formArray["p"] <= 10){
					...
					if ($al <> $lvl) {
						if ($av > 0) {
							...
							$plantitems .= "<lineitem x1=\"115\" y1=\"".$offsetx."\" x2=\"188\" y2=\"".$offsetx."\">blurb</lineitem>";
							$plantitems .= "<lineitem x1=\"198\" y1=\"".$offsetx."\" x2=\"254\" y2=\"".$offsetx."\">blurb</lineitem>";
							$plantitems .= "<lineitem x1=\"264\" y1=\"".$offsetx."\" x2=\"347\" y2=\"".$offsetx."\">blurb</lineitem>";
		...
		if(count($machineriesList)){
			foreach($machineriesList as $mkey => $machineries){
				...
				if($this->formArray["p"] <= 10){
					...
					$items .= "<textitem xpos=\"337\" ypos=\"".$offset."\" font=\"Helvetica\" size=\"10\" align=\"right\">".number_format($machineries->getAdjustedMarketValue(),2)."</textitem>";
					...
					$this->formArray["totalMarketValue"] += $machineries->getAdjustedMarketValue();
	...
	function displayTDDetails(){
		...
		if (!$xmlStr = $TDDetails->getTD("",$afsID,"","")){
			...
			if(!$domDoc = domxml_open_mem($xmlStr)) {
				...
				$this->formArray["previousAssessedValue"] = number_format($td->getpreviousAssessedValue(),2);
				$dates=explode("-",$td->getCityMunicipalAssessorDate());
				$fulldate = mktime(0,0,0,$dates[1],$dates[2],$dates[0]);
				$this->formArray["cityMunicipalAssessorDate"] = date("F j, Y",$fulldate);
	...
	function Main(){
		...
		if (!$xmlStr = $AFSDetails->getAFS($this->formArray["afsID"])){
			...
			if(!$domDoc = domxml_open_mem($xmlStr)) {
				...
				//$fp = fopen("printtd.log","w");
				$td = new TD;
				$td->selectRecord("",$this->formArray["afsID"]);
				//fwrite($fp,"afsID = ".$this->formArray["afsID"]."\r\n");
				
				$prsnID = $td->getProvincialAssessor();
				//fwrite($fp,"pa ID = ".$prsnID."\r\n");
				$prsn = new Person;
				$prsn->selectRecord($prsnID);
				$paname = $prsn->getFullName();
						
				$prsnID = $td->getCityMunicipalAssessor();
				//fwrite($fp,"ma ID = ".$prsnID."\r\n");
				$prsn = new Person;
				$prsn->selectRecord($prsnID);
				$maname = $prsn->getFullName();

				$this->formArray["municipalAssessor"] = $maname;
				$this->formArray["provincialAssessor"] = $paname;
				//fclose($fp);
