...
//		$sql = sprintf("select * from %s order by tdID desc %s;",TD_TABLE, $condition);
    ...
    function searchRecords($searchKey,$fields,$limit=""){
        ...
        if($fields["taxBeginsWithTheYear"]!="" && $fields["ceasesWithTheYear"]!=""){
			$condition = $condition." or ";
            $condition = $condition."(";
            $condition = $condition.$fields["taxBeginsWithTheYear"]." <= '".$searchKey."' AND ".$fields["ceasesWithTheYear"]." >= '".$searchKey."'";
            $condition = $condition.")";
        }

		$condition = $condition. " or ";
		$condition = $condition. "(";
		$condition = $condition.$fields["taxBeginsWithTheYear"]." like '%".$searchKey."%'";
		$condition = $condition. ") ";
		
		$sql = sprintf("SELECT tdID FROM %s",
		  TD_TABLE);		
    ...
    function countSearchRecords($searchKey,$fields,$limit=""){
		...
		$sql = sprintf("SELECT COUNT(tdID) as count FROM %s",
		  TD_TABLE);
