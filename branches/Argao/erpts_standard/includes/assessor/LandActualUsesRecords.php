<?php
class LandActualUsesRecords
{
	var $arrayList;
	var $domDocument;
	var $db;
	
	function LandActualUsesRecords(){
	...
	function setDomDocument(){
		...
		$domList = $this->domDocument->create_element("LandActualUsesList");
	...
	function parseDomDocument($domDoc){
		...
		if ($baseNode->has_child_nodes()){
			...
			while ($child){
				//if ($child->tagname=="LandActualUses") {
				if ($child->tagname) {
					$tempXmlStr = $domDoc->dump_node($child);
					$tempDomDoc = domxml_open_mem($tempXmlStr);
					$landActualUses = new LandActualUses;
					$landActualUses->parseDomDocument($tempDomDoc);
					$this->arrayList[] = $landActualUses;
				}
	...
	function selectRecords($condition=""){
		...
		//		LAND_ACTUALUSES_TABLE, $condition);
		...
		while ($this->db->next_record()) {
			$landActualUses = new LandActualUses;
			$landActualUses->selectRecord($this->db->f("landActualUsesID"));
			$this->arrayList[] = $landActualUses;
		}
	...
	function searchRecords($searchKey,$fields,$limit){
		...
		$sql = sprintf("select * from %s %s;",
				LAND_ACTUALUSES_TABLE, $condition.") ".$limit);
		//echo $sql;
		$this->setDB();
		$this->db->query($sql);
		while ($this->db->next_record()) {
	...
	function deleteRecords($landActualUsesIDArray=""){
		$landActualUses = new LandActualUses;
		$rows = 0;
		foreach ($landActualUsesIDArray as $key => $value){
			if ($landActualUses->deleteRecord($value)) $rows++;
		}
		return $rows;
	}
	...
	function updateStatus($statusIDArray=""){
	    ...
	    foreach($statusIDArray as $key => $value){
	        $landActualUses = new LandActualUses;
	        $landActualUses->selectRecord($value);
	        $landActualUses->status = "active";
	        $landActualUses->updateRecord();
	        $rows++;
	    }
	    return $rows;
	}
