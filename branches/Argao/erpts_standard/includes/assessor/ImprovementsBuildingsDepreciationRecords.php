<?php
class ImprovementsBuildingsDepreciationRecords
{
	var $arrayList;
	var $domDocument;
	var $db;
	
	function ImprovementsBuildingsDepreciationRecords(){
	...
	function setDomDocument(){
		...
		$domList = $this->domDocument->create_element("ImprovementsBuildingsDepreciationList");
	...
	function selectRecords($condition=""){
		...
		//		IMPROVEMENTSBUILDINGS_DEPRECIATION_TABLE, $condition);

		$sql = sprintf("select 
			improvementsBuildingsDepreciationID,
			code,
			reportCode,
			description,
			rangeLowerBound,
		...
		while ($this->db->next_record()) {
			$improvementsBuildingsDepreciation = new ImprovementsBuildingsDepreciation;
			$improvementsBuildingsDepreciation->selectRecord($this->db->f("improvementsBuildingsDepreciationID"));
			$this->arrayList[] = $improvementsBuildingsDepreciation;
		}
	...
	function searchRecords($searchKey,$fields,$limit){
		...
		$sql = sprintf("select * from %s %s;",
				IMPROVEMENTSBUILDINGS_DEPRECIATION_TABLE, $condition.") ".$limit);
		//echo $sql;
		$this->setDB();
		$this->db->query($sql);
		while ($this->db->next_record()) {
	...
	function deleteRecords($improvementsBuildingsDepreciationIDArray=""){
		$improvementsBuildingsDepreciation = new ImprovementsBuildingsDepreciation;
		$rows = 0;
		foreach ($improvementsBuildingsDepreciationIDArray as $key => $value){
			if ($improvementsBuildingsDepreciation->deleteRecord($value)) $rows++;
		}
		return $rows;
	}
