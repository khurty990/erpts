...
include_once("records/assessor/Barangay.php");
include_once("records/assessor/BarangayRecords.php");
include_once("records/assessor/MunicipalityCity.php");
include_once("records/assessor/MunicipalityCityRecords.php");
include_once("records/assessor/Province.php");
include_once("records/assessor/ProvinceRecords.php");

include_once("records/assessor/OD.php");
include_once("records/assessor/AFS.php");
include_once("records/assessor/TD.php");
include_once("records/assessor/LocationAddress.php");

include_once("records/assessor/LandActualUses.php");
include_once("records/assessor/PlantsTreesActualUses.php");
include_once("records/assessor/ImprovementsBuildingsActualUses.php");
include_once("records/assessor/MachineriesActualUses.php");

include_once("records/collection/BacktaxTD.php");

include_once("records/collection/Payment.php");
include_once("records/collection/PaymentRecords.php");
include_once("records/collection/Collection.php");
include_once("records/collection/CollectionRecords.php");
include_once("records/collection/Receipt.php");
include_once("records/collection/ReceiptRecords.php");
...
class ProvincialQuarterlyReportOnRPTCollectionsPrint{
	
	var $tpl;
	var $formArray;

	function ProvincialQuarterlyReportOnRPTCollectionsPrint($http_post_vars,$sess){
		$this->sess = $sess;
		$this->tpl = new rpts_Template(getcwd(),"keep");

		$this->tpl->set_file("rptsTemplate", "reports2B2.xml") ;
		$this->tpl->set_var("TITLE", "Provincial Quarterly Report On RPT Collections");
...
//					}
	...
	function Main(){
		if(!is_array($this->formArray["DBid"])){
			exit("no LGUs selected");
		}
		else{
			foreach($this->formArray["DBid"] as $dbID){
				$this->dbID = $dbID;
				$lgu = new LGU;
				$lgu->selectRecord($dbID);
				$this->dbName = $lgu->getLGUDB();
				$this->db = new DB_SelectLGU($this->dbName);
...
$provincialQuarterlyReportOnRPTCollectionsPrint = new ProvincialQuarterlyReportOnRPTCollectionsPrint($HTTP_POST_VARS,$sess);
$provincialQuarterlyReportOnRPTCollectionsPrint->Main();
?>
<?php page_close(); ?>