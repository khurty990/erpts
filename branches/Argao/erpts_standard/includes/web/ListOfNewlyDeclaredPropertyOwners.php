...
#####################################
class ListOfNewlyDeclaredPropertyOwners{
	
	var $tpl;
	function ListOfNewlyDeclaredPropertyOwners($sess){
		global $auth;
		...
		$this->tpl->set_file("rptsTemplate", "ListOfNewlyDeclaredPropertyOwners.htm") ;
		$this->tpl->set_var("TITLE", "List of Newly Declared Properites");
	}
	...
	function Main(){
		...
		$sql.= " (".OD_TABLE.".transactionCode = 'DC' OR ".OD_TABLE.".transactionCode = '')";

		$sql.= " AND ".OD_TABLE.".archive !=  'true'";
		...
		$this->tpl->set_var("totalNewlyDeclaredProperties", $count);
...
//*/
$listOfNewlyDeclaredPropertyOwners = new ListOfNewlyDeclaredPropertyOwners($sess);
$listOfNewlyDeclaredPropertyOwners->Main();
?>
<?php page_close(); ?>
