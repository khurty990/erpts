...
#####################################
class DailyTransactionListing{
	
	var $tpl;
	function DailyTransactionListing($sess){
		global $auth;
		...
		$this->tpl->set_file("rptsTemplate", "DailyTransactionListing.htm") ;
		$this->tpl->set_var("TITLE", "Daily Transaction Listing");
	}
	...
	function Main(){
		...
		$sql.= " AND ".OD_TABLE.".dateCreated >= '".strtotime(date("n/j/Y")." 00:00:00")."' ";
		$sql.= " AND ".OD_TABLE.".dateCreated <= '".strtotime(date("n/j/Y")." 23:59:59")."' ";
//		$sql.= " AND ".OD_TABLE.".archive !=  'true'";
		...
		$this->tpl->set_var("totalTransactionsToday", $count);
...
//*/
$dailyTransactionListing = new DailyTransactionListing($sess);
$dailyTransactionListing->Main();
?>
<?php page_close(); ?>
